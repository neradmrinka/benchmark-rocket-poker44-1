...
def _amount_bucket(value: float) -> str:
    """Coarse size class used by the sizing-signature features.

    The `return "xl"` used to sit indented inside the `<= 5.0` branch, so it was
    unreachable and every bet above 5bb fell off the end as an implicit None.
    That collapsed the validator's entire 6..126bb range into one class, which is
    exactly where a bot's replayed bet sizing is most visible. The ladder now runs
    to the top of payload_view._VISIBLE_BB_BUCKETS (126bb).
    """
