...
if __name__ == "__main__":
    ...
    # resolve the repo from this file's location, not from the box it was written on
    _MODEL = Path(__file__).resolve().parent
    for _p in (str(_MODEL), str(_MODEL.parent)):
    ...
    ex = load_examples()
    raw = extract_features_v2(ex[0].hands)
    san = extract_features_v2([prepare_hand_for_miner(h) for h in ex[0].hands])
    print(f"n_features={len(raw)}")
    nz_raw = sum(1 for v in raw.values() if v != 0.0)
    nz_san = sum(1 for v in san.values() if v != 0.0)
    print(f"nonzero on RAW chunk: {nz_raw}/{len(raw)}")
    print(f"nonzero on SANITIZED chunk: {nz_san}/{len(san)}  (should stay high, unlike old hero features)")
